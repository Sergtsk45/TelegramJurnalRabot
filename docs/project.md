...
- `GET /api/pdfs/:filename` — выдача сгенерированных PDF из `generated_pdfs/`.
  - Ограничение безопасности: принимается только **имя файла** (без директорий), только `*.pdf`, только безопасные символы (`A-Za-z0-9._-`). На невалидное имя возвращается `400`.
