...
- **Шаги выполнения**:
  - [x] Добавить `ALLOW_DESTRUCTIVE_ACTIONS` (env) и единый helper/функцию “можно ли деструктивно”
  - [x] Перевести под гейт:
    - [x] `DELETE /api/works`
    - [x] `POST /api/works/import` с `mode=replace`
    - [x] demo-seed (согласовать логику с `ENABLE_DEMO_SEED`)
  - [x] Обновить `/docs/project.md` (описать `ALLOW_DESTRUCTIVE_ACTIONS` и поведение)
  - [x] Обновить `/docs/changelog.md`
- **Acceptance Criteria**:
  - [x] В production без `ALLOW_DESTRUCTIVE_ACTIONS=true` все destructive операции запрещены/недоступны
  - [x] В dev поведение предсказуемое и документировано
- **Зависимости**: Нет
...
- **Статус**: Завершена
- **Описание**: Подтвердить “зелёную” типизацию `tsc` в актуальном окружении и зафиксировать результат в документации.
- **Шаги выполнения**:
  - [x] Установить зависимости (`npm ci`) в рабочем окружении
  - [x] Прогнать `npm run check` и подтвердить отсутствие ошибок TypeScript
  - [x] Зафиксировать результат в `/docs/changelog.md`
- **Acceptance Criteria**:
  - [x] `npm run check` проходит без ошибок
