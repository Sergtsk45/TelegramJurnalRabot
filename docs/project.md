...
- В production деструктивные операции отключены по умолчанию и требуют явного флага окружения `ALLOW_DESTRUCTIVE_ACTIONS=true`.
- Под это правило попадают:
  - `DELETE /api/works` (очистка ВОР),
  - `POST /api/works/import` с `mode=replace`,
  - demo-seed (`ENABLE_DEMO_SEED=true`).
- В UI очистка ВОР сопровождается предупреждением и обычно выполняется с query `resetSchedule=1` для безопасного сброса графика/актов, если ВОР используется как источник.
...
- **Server**
  ...
  - `ALLOW_DESTRUCTIVE_ACTIONS=true` — разрешить деструктивные операции в production (по умолчанию выключено).
  - `ENABLE_DEMO_SEED=true` — включить сидирование демо-работ в пустую БД. Выполнится только если деструктивные операции разрешены.
