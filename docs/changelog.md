...
## [2026-02-20] - Security: защита `GET /api/pdfs/:filename` и санация имён сгенерированных PDF

### Исправлено
- `server/routes.ts`: закрыт риск path traversal в `GET /api/pdfs/:filename` — добавлена строгая валидация имени файла (только безопасные символы и суффикс `.pdf`), выдача файла ограничена каталогом `generated_pdfs/`.
- `server/routes.ts`: имя файла при экспорте PDF больше не зависит напрямую от несанитизированного `template.code`/`actNumber` — добавлена санация частей имени, чтобы исключить запись файла в неожиданные пути.
